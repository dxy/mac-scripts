...
format_version() {
  # "Apple has a formalised version number structure based around the NumVersion
  # struct, which specifies a one- or two-digit major version, a one-digit minor
  # version, a one-digit "bug" (i.e. revision) version, a stage indicator (drawn
  # from the set development/prealpha, alpha, beta and final/release), and a
  # one-byte (i.e. having values in the range 0-255) pre-release version, which
  # is only used at stages prior to final."
